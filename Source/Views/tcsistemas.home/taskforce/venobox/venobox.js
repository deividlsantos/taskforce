...
(function($){
                  ...
                  obj.click(function(e){
                    ...
                    function checknav(){
                      ...
                      if(items.length > 1 && numeratio === true){
                        blocknum.php(items.index(obj)+1 + ' / ' + items.length);
                        blocknum.show();
                      }else{
                        blocknum.hide();
                      }
    ...
    function loadAjax(){
      ...
      cache: false
          ...
          content.php('<div class="vbox-inline">'+ msg +'</div>');
          updateoverlay(true);

      }).fail(function() {
          content.php('<div class="vbox-inline"><p>Error retrieving contents, please retry</div>');
          updateoverlay(true);
      })
    }
    ...
    /* -------- LOAD IFRAME -------- */
    function loadIframe(){
      content.php('<iframe class="venoframe" src="'+dest+'"></iframe>');
    ...
    function loadVimeo(autoplay){
      ...
      content.php('<iframe class="venoframe" webkitallowfullscreen mozallowfullscreen allowfullscreen frameborder="0" src="//player.vimeo.com/video/'+videoid+stringAutoplay+'"></iframe>');
      updateoverlay();
    }
    ...
    function loadYoutube(autoplay){
      ...
      var videoid = pezzi[pezzi.length-1];
      var stringAutoplay = autoplay ? "?autoplay=1" : "";
      content.php('<iframe class="venoframe" webkitallowfullscreen mozallowfullscreen allowfullscreen src="//www.youtube.com/embed/'+videoid+stringAutoplay+'"></iframe>');
      updateoverlay();
    }

    /* -------- LOAD INLINE -------- */
    function loadInline(){
      content.php('<div class="vbox-inline">'+$(dest).php()+'</div>');
      updateoverlay();
    }
